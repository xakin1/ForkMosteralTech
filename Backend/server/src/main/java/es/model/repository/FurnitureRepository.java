...
public interface FurnitureRepository
  ...
  @Query("SELECT p AS product, CASE WHEN f.id IS NOT NULL THEN TRUE ELSE FALSE END AS isFavourite FROM t_furniture p LEFT JOIN t_favourites f ON f.product.id = p.id AND f.appuser.id = :userId")
